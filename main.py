...
from config import CARD_IMGS_FOLDER
import json
...
def get_card_names(g_id):
    ...
    for i, card in enumerate(cards):
        # dynamically generate URL using url_for
        info['cards'][f'card{i}'] = url_for(
            'static',
            filename=f'{CARD_IMGS_FOLDER}/{card.value}_of_{card.suit}.png'
        )
    return info
...
@app.route('/')
def home():
    backside = url_for('static', filename=f'{CARD_IMGS_FOLDER}/backside.png')
    hold_img = url_for('static', filename=f'{CARD_IMGS_FOLDER}/hold.png')
    return render_template('index.html', backside=backside, hold_img=hold_img)
